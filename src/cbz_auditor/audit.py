...
def audit_archive(
    path: Path, max_pages: int = 10_000, max_uncompressed_mb: int = 2_000
) -> Result:
    ...
    try:
        ...
        return Result(
            str(archive),
            (),
            (Finding("error", "invalid-archive", f"Archive cannot be read: {exc}"),),
        )
    ...
    for entry in entries:
        ...
        if pure is None:
            findings.append(
                Finding(
                    "error",
                    "unsafe-path",
                    "Archive entry uses an unsafe path.",
                    entry.name,
                )
            )
            continue
        if pure.suffix.casefold() not in IMAGE_EXTENSIONS:
            if (
                pure.name.casefold() not in {"comicinfo.xml", ".ds_store"}
                and "__MACOSX" not in pure.parts
            ):
                findings.append(
                    Finding(
                        "warning",
                        "non-image-entry",
                        "Unexpected non-image file in archive.",
                        entry.name,
                    )
                )
            continue
    ...
    if not pages:
        findings.append(
            Finding("error", "no-pages", "No readable page images were found.")
        )
        ...
        if page.width < 500 or page.height < 500:
            findings.append(
                Finding(
                    "warning",
                    "small-page",
                    f"Page is unusually small at {page.width}×{page.height}.",
                    page.name,
                )
            )
        if page.mean_brightness <= 2 or page.mean_brightness >= 253:
            findings.append(
                Finding(
                    "warning",
                    "nearly-blank",
                    f"Page is nearly uniform at mean brightness {page.mean_brightness:.1f}.",
                    page.name,
                )
            )
        ...
        if ratio >= 1.35:
            findings.append(
                Finding(
                    "info",
                    "possible-spread",
                    f"Wide page may be a double-page spread ({page.width}×{page.height}).",
                    page.name,
                )
            )
    findings.sort(
...
def _read_entries(
    path: Path, max_entries: int, max_uncompressed_mb: int
        ...
        with rarfile.RarFile(path) as source:
            ...
            for item in infos:
                try:
                    ...
                    raise ValueError(
                        "RAR/CBR needs UnRAR, 7-Zip, unar, or bsdtar installed and available on PATH"
                    ) from exc
        return
    if suffix in {".cb7", ".7z"}:
        with py7zr.SevenZipFile(path, mode="r") as source:
            seven_infos = [item for item in source.list() if not item.is_directory]
            _enforce_limits(
                ((item.filename, int(item.uncompressed or 0)) for item in seven_infos),
                max_entries,
                max_bytes,
            )
            if any(not item.is_file or item.is_symlink for item in seven_infos):
                raise ValueError("7Z links and non-regular entries are not accepted")
            unsafe = [
                item.filename
                for item in seven_infos
                if _safe_name(item.filename) is None
            ]
            targets = [
                item.filename
                for item in seven_infos
                if _safe_name(item.filename) is not None
            ]
        for name in unsafe:
        ...
        with tempfile.TemporaryDirectory(prefix="cbz-audit-") as temporary:
            ...
            for name in targets:
                ...
                if root not in extracted.parents or not extracted.is_file():
                    raise ValueError(
                        f"7Z entry did not extract to a safe regular file: {name}"
                    )
...
def _enforce_limits(
    entries: Iterator[tuple[str, int]], max_entries: int, max_bytes: int
) -> None:
    values = list(entries)
    ...
    if total > max_bytes:
        raise ValueError(
            f"Uncompressed archive size exceeds {max_bytes // (1024 * 1024)} MB limit."
        )
...
def _duplicates(pages: list[Page]) -> list[Finding]:
    ...
    return [
        Finding(
            "warning",
            "duplicate-page",
            "Identical page data also appears as: " + ", ".join(names[1:]),
            names[0],
        )
...
def _dimension_outliers(pages: list[Page]) -> list[Finding]:
    ...
    return [
        Finding(
            "warning",
            "dimension-outlier",
            f"Page area differs substantially from the archive median ({page.width}×{page.height}).",
            page.name,
        )
        for page in pages
        if (page.width * page.height) < median_area * 0.35
        or (page.width * page.height) > median_area * 3
    ]
...
def _numbering_gaps(pages: list[Page]) -> list[Finding]:
    ...
    gaps = [
        number for number in range(values[0], values[-1] + 1) if number not in values
    ]
    return (
        [
            Finding(
                "warning",
                "numbering-gap",
                "Possible missing page numbers: "
                + ", ".join(map(str, gaps[:25]))
                + ("…" if len(gaps) > 25 else ""),
            )
        ]
        if gaps
